...
export const addHost = async (req, res) => {
    try {
        ...
        if (!user_id) {
            return res.status(HTTP_STATUS.BAD_REQUEST).json({ message: "user_id is required" });
        }
        
        const result = await eventService.addHost(
            req.params.eventId,
            req.user._id,
            user_id
        );
        ...
        if (!result.success) {
            ...
            if (result.error === 'CANNOT_ADD_CREATOR') {
                return res.status(HTTP_STATUS.BAD_REQUEST).json({ message: "Cannot add creator as co-host" });
            }
            if (result.error === 'ALREADY_HOST') {
                return res.status(HTTP_STATUS.BAD_REQUEST).json({ message: "User is already a host" });
            }
        }
        
        return res.status(HTTP_STATUS.CREATED).json({ host: result.host });
    } catch (error) {
        return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({ message: error.message });
    }
};
...
export const removeHost = async (req, res) => {
    try {
        const result = await eventService.removeHost(
            req.params.eventId,
            req.user._id,
            req.params.userId
        );
        ...
        if (!result.success) {
            ...
            if (result.error === 'NOT_FOUND') {
                return res.status(HTTP_STATUS.NOT_FOUND).json({ message: "Host not found" });
            }
        }
        
        return res.status(HTTP_STATUS.OK).json({ message: "Host removed successfully" });
    } catch (error) {
        return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({ message: error.message });
    }
};
...
export const getHosts = async (req, res) => {
    try {
        const hosts = await eventService.getHosts(req.params.eventId);
        res.status(HTTP_STATUS.OK).json({ count: hosts.length, hosts });
    } catch (error) {
        return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({ message: error.message });
    }
};
...
export const getEventsByHost = async (req, res) => {
    try {
        const events = await eventService.getEventsByHost(req.user._id);
        res.status(HTTP_STATUS.OK).json({ count: events.length, events });
    } catch (error) {
        return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({ message: error.message });
    }
